...
```
mvn package && AWS_ACCESS_KEY=secret AWS_SECRET_KEY=secret java -jar target/s3-cf-service-broker-2.1.0-SNAPSHOT.jar
```
...
```
mvn package
cf push s3-cf-service-broker -p target/s3-cf-service-broker-2.1.0-SNAPSHOT.jar --no-start
